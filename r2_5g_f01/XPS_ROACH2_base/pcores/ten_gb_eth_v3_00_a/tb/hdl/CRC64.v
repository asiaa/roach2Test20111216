...
module CRC64 (
   ...
   localparam CRC_INIT_HACK = 32'hFFFFFFFF;
...
//   wire 	 crcclk_int;
   ...
   initial
    $display("CRC_INIT = %x", CRC_INIT_HACK);
   

   function [31:0] bit_rev;
   input [31:0] in;
   integer i;
   begin
   ...
   always @(posedge CRCCLK) begin
     if (CRCDATAVALID) begin
       $display("crcdata = %x, width = %b, reset = %b", CRCIN, CRCDATAWIDTH, CRCRESET);
       #1
       $display("real crc = %x", bit_rev(byte_rev(CRCOUT)));
     end
   end

   always @(*)
     begin
	     crcout_out = {!crcreg[24],!crcreg[25],!crcreg[26],
