...
def write_all_ibobs_ctrl(input_sel=0, use_sram_tvg=False, use_fft_tvg=False, arm_rst=False, sync_rst=False, fft_shift=1023):
    ...
    rv = []
    for b in bees: b.write('ibob_addr', (2**32)-1)
    for b in bees: b.write('ibob_data', value)
    for b in bees: rv.append(b.write('ibob_addr',8192))
    return rv
...
def arm():
    write_all_ibobs_ctrl(input_sel=0, use_sram_tvg=False, use_fft_tvg=False, arm_rst=True, sync_rst=False, fft_shift=int(fft_shift,16))
    return check_result(write_all_ibobs_ctrl(input_sel=0, use_sram_tvg=False, use_fft_tvg=False, arm_rst=False, sync_rst=False, fft_shift=int(fft_shift,16)))
...
num_xeng_per_fpga = config['x_per_fpga']
...
for b,bee in enumerate(bees):
    for f,fpga in enumerate(fpgas):
        if (bee.write('base_ant', ant, fpgas=[fpga])[fpga]['status'] != 'SUCCESS'):
            print 'Failed to program %s on %s'%(fpga,servers[b])     
        else:
            print('%i'%ant),
            sys.stdout.flush()
        ant += num_xeng_per_fpga
print ('''done''')
...
for b,bee in enumerate(bees):
    for f,fpga in enumerate(fpgas):
        ip = gbe_start_ip + b*4 + f 
        if (bee.write('gbe_ip', ip, fpgas=[fpga])[fpga]['status'] != 'SUCCESS'):
            print('''Failed to program %s on %s''' %(fpga,servers[b]))     
print ('''done''')
...
for b,bee in enumerate(bees):
    for fpga,f in enumerate(fpgas):
        new_gbe_conf=get_gbe_conf(port=gbe_port,start_addr=gbe_start_ip,fpga=(b*4)+fpga)
        if (bee.binary_write('gbe', new_gbe_conf, fpgas=[f])[f]['status'] != 'SUCCESS'):
            print('''Failed to program %s on %s''' %(f,servers[b]))     
        if (bee.binary_read('gbe',num=-1)[f]['data'] != new_gbe_conf): 
            print('''Failed verification for %s on %s''' %(f,servers[b]))
        fpga += 1
print('ok')
