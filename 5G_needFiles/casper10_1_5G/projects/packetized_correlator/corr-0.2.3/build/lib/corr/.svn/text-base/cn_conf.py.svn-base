...
class CorrConf:    
    ...
    def read_all(self):
        ...
        try:
            ...
            return 'ERR Processing the reveiver section'

        # get the antenna info:
        self.config['antennas']=dict()
        self.config['antennas']['EQ_polys']=[]
        for ant in range(self.config['n_ants']):
            ant_eq_str=self.get_line('antennas','EQ_poly_%i'%(ant))
            if (ant_eq_str):
                self.config['antennas']['EQ_polys'].append([int(coef) for coef in ant_eq_str.split(LISTDELIMIT)])
            else:
