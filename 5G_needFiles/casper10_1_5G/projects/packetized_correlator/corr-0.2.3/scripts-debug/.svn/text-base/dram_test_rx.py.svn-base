...
#            print self.data[i/16]
    ...
    def process_packets(self,num_listeners=1, port=8888, num_acc=8192, look_non_zeros=False, payload_len=8192, hdr_size=21, verbose=False, req_x_eng=0):
        ...
        correlator=corr.packet.CasperN_RX_correlator(port=port, payload_len=payload_len, hdr_size=hdr_size, endian='>')
        ...
        while not self.quit:
            ...
            if not p == None:
                ...
                if verbose:
                    print ' Got index %i for X engine %i, timestamp %d with labelled length %i. Raw length %i, %i vectors.' %(p['index'],p['xengine_num'],p['time'],p['data_size'],len(raw_p),len(p['data']))
                
                if self.pkt_data == None:
                    if p['index']==0:
                        print 'Got first packet for X engine ',p['xengine_num']
                        self.pkt_data = p
                        self.cnt = 1
                    else:
                        if verbose:
                    ...
                    if self.cnt != p['index']:
                      print 'Got index %i for x engine %i instead of %i for x engine %i'%(p['index'], p['xengine_num'], self.cnt, self.prev_x_eng)
                      self.cnt = 0
                      #self.quit = True
                    else:
                    ...
                    if (len(self.pkt_data['data']) == total_p_size):
                        if (p['xengine_num'] == self.prev_x_eng):
                            self.apply_data()
                            print "\nGot all the data for X engine %i."%self.prev_x_eng
                            if (p['xengine_num']==self.req_x_eng):
                                if look_non_zeros:
