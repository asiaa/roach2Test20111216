#! /usr/bin/env python
import time, os, threading, subprocess, corr.sim_cn_data, corr.packet,struct
            ...
            for i, b in enumerate(self.bof_ints):
			    ...
			    #print ('%i,b[%s]='%(x_num,(dev+'_addr'))),num
                    ...
                    while n_integrations >= 2 and len(data) > 0 and not self._quit:
                        ...
                        pkt = self.packet.pack_from_prms((t*1000000), x_num, cnt*payload_len, 0, d)
                        self.sock.tx(pkt)
...
if __name__ == '__main__':
    ...
    p.add_option('-s', '--sim', dest='sim', action='store_true',
        help='Simulate data')
    p.add_option('-l', '--payload_len', dest='payload_len', type='int',default=512,
        help='The length in bytes of each packet (data or payload only). Default 512')
    ...
    if opts.sim:
        ...
        def equalize(data):
            ...
            bof_ints[fpga].write('ibob_addr',(2**32)-1)
            for c in range(nchan):
                gain = 0
                for i, p in enumerate(poly): gain += p * c**(len(poly) - i - 1)
                if gain < 0: gain = 0
                if gain >= 2**17: gain = 2**17 - 1
                bof_ints[fpga].write('ibob_data', (c << 20 | 1 << 17 | int(gain)))
                bof_ints[fpga].write('ibob_addr', c)
                bof_ints[fpga].write('ibob_addr', c+2048)
            print 'done.'
