#! /usr/bin/env python
"""
Update tmp.png from the database file specified in cgi arguments and generate HTML output through stdout.

to do:
parameterize the hard-coded stuff and fix all the broken functionality.
"""
...
import cgi, cgitb, os, shutil, sys, time
cgitb.enable()

os.environ['HOME']='/public_temp'
import matplotlib
matplotlib.use('Agg')
import corr.plotdb
...
if (mode == None or mode == 'auto'):
    ...
    db.plot_auto(outfile='/public_temp/tmp.png')

elif (mode == 'real'):
    print """<title>Real values: Cross-Correlations</title>\n</head><h3>Auto Correlations for all antennas:</h3><body><br>  """
    db.plot_real(outfile='/public_temp/tmp.png',chan1=int(chan1),chan2=int(chan2))

elif (mode == 'cross'):
    print "<title>Cross-Correlations</title>\n</head><h3>Cross Correlation between antennas %s and %s:</h3>"%(chan1,chan2)
    db.plot_cross(outfile='/public_temp/tmp.png',chan1=int(chan1),chan2=int(chan2))

elif (mode == 'raw'):
    print "<title>Raw Data</title>\n</head><h3>Raw correlator output between antennas %s and %s:</h3>"%(chan1,chan2)
    db.plot_raw(outfile='/public_temp/tmp.png',chan1=int(chan1),chan2=int(chan2))

elif (mode == 'amps'):
    print "<title>Amplitudes</title>\n</head><h3>Amplitude output between antennas %s and %s:</h3>"%(chan1,chan2)
    db.plot_amps(outfile='/public_temp/tmp.png',chan1=int(chan1),chan2=int(chan2))



print '''<img class="bordered" src="/public/tmp.png">'''
