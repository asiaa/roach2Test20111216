...
try:
    ...
    target_fpga = ant/c.config['n_ants_per_xaui']/c.config['n_xaui_ports_per_fpga']
    target_xaui = ant/c.config['n_ants_per_xaui']%c.config['n_xaui_ports_per_fpga']
    xaui_ant=ant/c.config['n_ants_per_xaui']/c.config['n_xaui_ports_per_fpga']
    ...
    while offset < (n_chans-1):
        ...
        print 'Capturing from frequency offset %i. Triggering at word offset %i'%(offset,offset*pkt_len*c.config['n_ants_per_xaui'])
        ...
        sys.stdout.flush()
        bram_oob=dict()
        bram_oob={'raw':struct.unpack('>%iL'%(len(bram_dmp[brams[2]])/4),bram_dmp[brams[2]])}
        ...
        skip_indices=[]
        print c.servers[target_fpga] + ': '
        pkt_hdr_idx = -1
        for i in range(0,len(bram_dmp[brams[0]])/4):
#            if opts.verbose:
...
#                print '[%s @ %4i]: %016X %s'%(c.servers[f],i,pkt_64bit,hdr_str)
            ...
            if bram_oob['linkdn'][i]:
                print '[%s] LINK DOWN AT %i'%(c.servers[target_fpga],i)
            elif bram_oob['adc'][i]:
                adc_amps = struct.unpack('>II',bram_dmp['bram_msb'][(4*i):(4*i)+4]+bram_dmp['bram_lsb'][(4*i):(4*i)+4])
                print '[%s] ADC amplitude update at index %i: '%(c.servers[target_fpga],i),adc_amps
    ...
    #            print ('EOF RECEIVED')
                ...
                if pkt_len-len(skip_indices) != (packet_len+1):
                    ...
                    if opts.verbose: print '[%s] [Pkt@ %4i Len: %2i]     (MCNT %12u ANT: %1i, Freq: %4i)    {4 bit: Qr: %1.2f Qi: %1.2f Ir %1.2f Ii: %1.2f}'%(c.servers[f],\
                        pkt_hdr_idx,\
