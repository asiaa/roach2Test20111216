...
class FpgaClient(BlockingClient):
    ...
    def tap_start(self, device_name,mac,ip,port):
        ...
        self._logger.info("Starting tgtap driver instance: %s %s %s %s %s"%("tap-start", device_name, ip_str, port_str, mac_str))
        reply, informs = self._request("tap-start", device_name, ip_str, port_str, mac_str)
        if reply.arguments[0]=='ok': return
        else: raise RuntimeError("Failure starting tap device %s with mac %s, %s:%s"%(device_name,mac_str,ip_str,port_str))

    def tap_stop(self, device_name):
        """Stop a TAP driver.
        ...
        reply, informs = self._request("tap-stop", device_name)
        if reply.arguments[0]=='ok': return
        else: raise RuntimeError("Failure stopping tap device %s."%(device_name))
    ...
    def read_dram(self, size, offset=0,verbose=False):
        ...
        while n_reads < size:
            ...
            local_data=(self.read('dram_memory',local_reads,local_offset))
