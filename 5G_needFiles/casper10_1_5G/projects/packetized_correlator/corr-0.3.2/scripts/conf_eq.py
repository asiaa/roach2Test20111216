...
def read_int_all(register):
    """Read register on all fpgas. 
    Returns 'ok' or string outlining failure."""
    for f,fpga in enumerate(fpgas):
        out = fpga.read_int(register)
        if out['status'] != ('SUCCESS'):
            return('''Failed to read %s:%s-%s''' %(servers[f],out['status'],out['data']))
        else:
            print 'fpga%i EQ:%i'%(f,out['data']),
    return('''ok''')
    ...
    for xaui in range(n_xaui_ports_per_fpga):
        for antenna in range(n_ants_per_xaui):
            for c, g in zip(chans, gains):
                ...
                if opts.verbose: print 'Wrote %i for channel %i on fpgas, xaui%i antenna %i.'%(int(g),int(c),xaui,int(antenna))
    return 'ok'
...
def apply_eq(fn,xaui,ant,chans, gains):
    ...
    for c, g in zip(chans, gains):
        ...
        if opts.verbose: print 'Wrote %i for channel %i on %s, xaui%i antenna %i.'%(int(g),int(c),servers[fn],xaui,int(ant))
    return 'ok'

def read_eq(chans):
    """Read equalization coefficients on the IBOB.
    EQ channels are in the first blocks of address space."""
    for xaui in range(n_xaui_ports_per_fpga):
        for antenna in range(n_ants_per_xaui):
            for c in chans:
                print '\nEQ Coefficents for channel %i on xaui%i Antenna %i.'%(int(c),xaui,int(antenna)),
                response=read_int_all('ibob_data%i'%(xaui))
                if response != 'ok': return response 
    return 'ok'
...
if __name__ == '__main__':
    ...
    p.add_option('-r', '--read', dest='read', action='store_true', default=False,
        help='''Read the current ibobs' equaliser channels''')
    p.add_option('-v', '--verbose', dest='verbose', action='store_true', default=False,
        help='''Verbose mode output, recommended''')
...
if opts.init_eq>0:
    ...
    print n_chans
    print 'Reading all current ibobs\' %i channels\' gains...'%(n_chans)
    sys.stdout.flush()
    print read_eq(range(n_chans))
