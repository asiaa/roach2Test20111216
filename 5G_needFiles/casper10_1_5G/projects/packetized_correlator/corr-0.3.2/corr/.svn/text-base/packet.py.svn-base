...
class CasperN_Packet:
    ...
    def pack(self, d):
        ...
        if not d.has_key('raw_data'):
            ...
            #print 'Targetted size of %i values. Actual: %i bytes.'%(len(d['data']),len(d['raw_data']))
...
class CasperN_TX_Socket(UDP_TX_Socket, CasperN_Packet):
    ...
    def send_packet(self, t, xeng, cnt, flags, data):
        ...
        return self.tx(self.pack_from_prms(t, xeng, cnt, flags, data))
...
class UDP_RX_Socket(socket.socket):
    ...
    def __init__(self,port=8888,timeout=60):
...
class CasperN_RX_correlator(UDP_RX_Socket):
    def __init__(self, port=8888, buffer_size=8192,timeout=60):
        self.sock = UDP_RX_Socket(port=port,timeout=timeout)
