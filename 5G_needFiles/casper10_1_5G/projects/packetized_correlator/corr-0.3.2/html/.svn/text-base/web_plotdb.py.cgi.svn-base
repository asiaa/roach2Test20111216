...
working_dir = dbfile.rpartition('/')
dbfile = working_dir[2]
...
try: max = form['max'].value
except KeyError: max=None

try: dyn_range = form['dyn_range'].value
except KeyError: dyn_range=None
...
if (mode == None or mode == 'auto'):
    ...
    print """<title>Auto-Correlations</title>\n</head><h3>Auto Correlations for all antennas:</h3><body><br>  """
    if dyn_range is None: db.plot_auto2(outfile='%s'%rand_file)
    else: db.plot_auto2(outfile='%s'%rand_file, max=float(max), dyn_range=float(dyn_range))
