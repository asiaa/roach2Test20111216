...
INCOMPLETE VERBOSE OUTPUT 27 OCT 2008
...
dev_name = 'reord0_snap_wr'

# OOB signalling bit offsets:
wr_addr_bit_width = 11
wr_addr_bit_offset = 53
rcvd_bit = 52
flag_bit = 51
wr_data_bit_width = 16
wr_data_bit_offset = 35
we_bit = 34
eof_bit = 33
mcnt_bit_width = 13
mcnt_bit_offset = 20
...
din_mask = (2**(din_bit_width+din_bit_offset)) -(2**din_bit_offset)
ant_mask = (2**(ant_bit_width+ant_bit_offset)) -(2**ant_bit_offset)
mcnt_mask = (2**(mcnt_bit_width+mcnt_bit_offset)) -(2**mcnt_bit_offset)
wr_data_mask = (2**(wr_data_bit_width+wr_data_bit_offset)) -(2**wr_data_bit_offset)
wr_addr_mask = (2**(wr_addr_bit_width+wr_addr_bit_offset)) -(2**wr_addr_bit_offset)
...
for f,fpga in enumerate(fpgas):
    ...
    bram_oob[f].update({'wr_data':[(i&wr_data_mask)>>wr_data_bit_offset for i in bram_oob[f]['raw']]})
    bram_oob[f].update({'wr_addr':[(i&wr_addr_mask)>>wr_addr_bit_offset for i in bram_oob[f]['raw']]})
    bram_oob[f].update({'din':[(i&din_mask)>>din_bit_offset for i in bram_oob[f]['raw']]})
...
if opts.verbose:
    for f,fpga in enumerate(fpgas):
        for i in range(bram_size[f]['data']+1):
            ...
            print '[%s] IDX: %6i MCNT: %6i. ANT: %4i.  Contents: %16x'%(servers[f]i,pkt_ip_str,pkt_mcnt,pkt_ant,pkt_64bit),
            if bram_oob[f]['we'][i]: print '[we]',
...
for f,fpga in enumerate(fpgas):
    ...
    for i in range(bram_size[f]['data']+1):
        if bram_oob[f]['eof'][i]:
            ...
            print '[%s] EOF at %4i. MCNT: %6i. Freq: %4i ANT: %4i. Len: %3i. '%(servers[f],i,pkt_mcnt,pkt_freq,pkt_ant,pkt_len),
