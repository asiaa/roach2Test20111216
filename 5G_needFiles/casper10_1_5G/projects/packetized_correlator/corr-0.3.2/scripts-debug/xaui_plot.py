...
Grabs the contents of "snap_xaui" for plotting.
Assumes 4 bit quantisation.
Assumes the correlator is already initialsed and running etc.

'''
import corr, time, numpy, struct, sys, stats, pylab
...
def feng_unpack(f,hdr_index,pkt_len,skip_indices):
    ...
    for pkt_index in range(1,(pkt_len)):
        ...
        for offset in range(0,64,16):
            ...
            arr_polQ_r.append( (float(((numpy.int8(polQ_r << 4)>> 4)))/(2**binary_point))**2)
            arr_polQ_i.append( (float(((numpy.int8(polQ_i << 4)>> 4)))/(2**binary_point))**2)
            arr_polI_r.append( (float(((numpy.int8(polI_r << 4)>> 4)))/(2**binary_point))**2)
            arr_polI_i.append( (float(((numpy.int8(polI_i << 4)>> 4)))/(2**binary_point))**2)
    ...
    return {'pkt_mcnt': pkt_mcnt,\
            ...
            'arr_polQ_r':arr_polQ_r,\
            'arr_polQ_i':arr_polQ_i,\
            'arr_polI_r':arr_polI_r,\
            'arr_polI_i':arr_polI_i,\
...
#            print ('EOF RECEIVED')
            ...
            """
            print '[%s] [Pkt@ %4i Len: %2i]     (MCNT %12u ANT: %1i, Freq: %4i)    {4 bit: Qr: %1.2f (%2.1f bits) Qi: %1.2f (%2.1f bits) Ir %1.2f (%2.1f bits) Ii: %1.2f (%2.1f bits)}'%(servers[f],\
                pkt_hdr_idx,\
            ...
            """
            freqs.append(feng_unpkd_pkt['pkt_mcnt']%n_chans)
            level_polQ_r.append(feng_unpkd_pkt['level_polQ_r'])
            level_polQ_i.append(feng_unpkd_pkt['level_polQ_i'])
            level_polI_r.append(feng_unpkd_pkt['level_polI_r'])
            level_polI_i.append(feng_unpkd_pkt['level_polI_i'])
            arr_polQ_r.extend(feng_unpkd_pkt['arr_polQ_r'])
            arr_polQ_i.extend(feng_unpkd_pkt['arr_polQ_i'])
            arr_polI_r.extend(feng_unpkd_pkt['arr_polI_r'])
            arr_polI_i.extend(feng_unpkd_pkt['arr_polI_i'])
            #print feng_unpkd_pkt['level_polQ_r'],feng_unpkd_pkt['level_polQ_i'],feng_unpkd_pkt['level_polI_r'],feng_unpkd_pkt['level_polI_i']
    ...
    #print level_polQ_r, level_polQ_i, level_polI_r, level_polI_i
    #print arr_polQ_r, arr_polQ_i, arr_polI_r, arr_polI_i
    #print len(arr_polQ_r), len(arr_polQ_i), len(arr_polI_r), len(arr_polI_i)
    pylab.plot(arr_polQ_r, label="Q_r")
    pylab.plot(arr_polQ_i, label="Q_i")
    pylab.plot(arr_polI_r, label="I_r")
    pylab.plot(arr_polI_i, label="I_i")
    pylab.legend()
    pylab.show()
    s = raw_input("[Press Enter for Next Plot]")
    pylab.clf()
