...
if opts.verbose:
    for f,fpga in enumerate(fpgas):
        for i in range(bram_size[f]['data']+1):
            ...
            if bram_oob[f]['loop_ack'][i]: print '[Loopback]',
            print 'IDX: %i IP: %s. MCNT: %i. ANT: %i.  Contents: %x'%(i,pkt_ip_str,pkt_mcnt,pkt_ant,pkt_64bit),
...
for f,fpga in enumerate(fpgas):
    ...
    for i in range(bram_size[f]['data']+1):
        if bram_oob[f]['eof'][i]:
            ...
            print '[%s] EOF at %4i. IP: %12s. MCNT: %i. ANT: %i. Len: %3i. '%(servers[f],i,pkt_ip_str,pkt_mcnt,pkt_ant,i-prev_eof_index),
            if bram_oob[f]['gbe_ack'][i]: print '[GBE]',
            elif bram_oob[f]['loop_ack'][i]: print '[Loopback]',
