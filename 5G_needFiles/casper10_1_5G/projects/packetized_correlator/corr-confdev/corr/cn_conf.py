...
class CorrConf:    
    ...
    def read_all(self):
        ...
        if (ant_ord_str):
            ...
            print order
            print len(order)
            if len(order) == self.config['n_ants']*2:
                try:
                    self.config['antennas']['order']= [{'id':ant.split(PORTDELIMIT)[0],'pol':ant.split(PORTDELIMIT)[1]} for ant in order]
                except:
        ...
        self.config['int_time'] = float(self.config['n_chans'])*self.config['xeng_acc_len']*self.config['acc_len']/(self.config['adc_clk']*1e9/self.config['ddc_decimation'])
        self.config['cent_freq'] = self.config['adc_clk']*1e9*self.config['ddc_mix_freq']
        self.config['bandwidth'] = self.config['adc_clk']*1e9/self.config['ddc_decimation']
