...
'''
Grabs the contents of "snap_gbe_mux" for analysis.
...
dev_name = 'snap_gbe_mux'

# OOB signalling bit offsets:
ip_addr_bit_width = 32-10
ip_addr_bit_offset = 10
eof_bit = 9
in0_ip_full_bit=8
in0_data_full_bit=7
in0_ip_empty_bit=6
in0_data_empty_bit=5
in1_ip_full_bit=4
in1_data_full_bit=3
in1_ip_empty_bit=2
in1_data_empty_bit=1
in_sel_bit = 0
...
def vacc_unpack(f,hdr_index,pkt_len):
    pkt_64bit = struct.unpack('>Q',bram_dmp['bram_msb'][f]['data'][(4*hdr_index)+0:(4*hdr_index)+4]+bram_dmp['bram_lsb'][f]['data'][(4*hdr_index):(4*hdr_index)+4])[0]
    pkt_time = pkt_64bit

    pkt_64bit = struct.unpack('>Q',bram_dmp['bram_msb'][f]['data'][(4*hdr_index)+4:(4*hdr_index)+8]+bram_dmp['bram_lsb'][f]['data'][(4*hdr_index)+4:(4*hdr_index)+8])[0]
    pkt_xeng = (pkt_64bit&((2**64)-(2**32)))>>32
    pkt_offset =  pkt_64bit&((2**32)-1)

    pkt_64bit = struct.unpack('>Q',bram_dmp['bram_msb'][f]['data'][(4*hdr_index)+8:(4*hdr_index)+12]+bram_dmp['bram_lsb'][f]['data'][(4*hdr_index)+8:(4*hdr_index)+12])[0]
    pkt_flags = (pkt_64bit&((2**64)-(2**32)))>>32
    pkt_data_len =  pkt_64bit&((2**32)-1)
    return{'pkt_time':pkt_time,'pkt_xeng':pkt_xeng,'pkt_offset':pkt_offset,'pkt_flags':pkt_flags,'pkt_data_len':pkt_data_len}
...
for f,fpga in enumerate(fpgas):
    ...
    bram_oob[f].update({'in0_ip_full':[bool(i&(2**in0_ip_full_bit)) for i in bram_oob[f]['raw']]})
    bram_oob[f].update({'in0_data_full':[bool(i&(2**in0_data_full_bit)) for i in bram_oob[f]['raw']]})
    bram_oob[f].update({'in0_ip_empty':[bool(i&(2**in0_ip_empty_bit)) for i in bram_oob[f]['raw']]})
    bram_oob[f].update({'in0_data_empty':[bool(i&(2**in0_data_empty_bit)) for i in bram_oob[f]['raw']]})
    bram_oob[f].update({'in1_ip_full':[bool(i&(2**in1_ip_full_bit)) for i in bram_oob[f]['raw']]})
    bram_oob[f].update({'in1_data_full':[bool(i&(2**in1_data_full_bit)) for i in bram_oob[f]['raw']]})
    bram_oob[f].update({'in1_ip_empty':[bool(i&(2**in1_ip_empty_bit)) for i in bram_oob[f]['raw']]})
    bram_oob[f].update({'in1_data_empty':[bool(i&(2**in1_data_empty_bit)) for i in bram_oob[f]['raw']]})
    bram_oob[f].update({'in_sel':[bool(i&(2**in_sel_bit)) for i in bram_oob[f]['raw']]})
...
for f,fpga in enumerate(fpgas):
    ...
    for i in range(bram_size):
        if opts.verbose:
            print 'Input: %i'%bram_oob[f]['in_sel'][i],
            ...
            print '[%s] IDX: %i IP: %s Contents: %x'%(servers[f],i,pkt_ip_str,pkt_64bit),
            if bram_oob[f]['eof'][i]: print 'EOF!'
            else: print '' 
            

        if bram_oob[f]['in0_ip_full'][i]:
            print 'ERR: Input0 IP buffer overflow  at index %i'%i
        if bram_oob[f]['in1_ip_full'][i]:
            print 'ERR: Input1 IP buffer overflow  at index %i'%i
        if bram_oob[f]['in0_data_full'][i]:
            print 'ERR: Input0 data buffer overflow  at index %i'%i
        if bram_oob[f]['in1_data_full'][i]:
            print 'ERR: Input1 data buffer overflow  at index %i'%i

        if bram_oob[f]['eof'][i]:
            pkt_ip_str = ip2str(bram_oob[f]['ip_addr'][i])
            print '[%s] EOF at %4i. Dest: %12s. Len: %3i. Pkt type:'%(servers[f],i,pkt_ip_str,i-prev_eof_index),
            ...
            if bram_oob[f]['in_sel'][i] == 0:
                print 'F engine exchange',
            ...
            elif bram_oob[f]['in_sel'][i] == 1:
                print 'Accumulated Xeng output',
                vacc_unpkd_pkt=vacc_unpack(f,hdr_index,pkt_len)
                print 'HDR @ %4i. Timestamp: %15i Xeng: %4i Offset: %4i Flags: %4i. Data Len: %i bytes'%(hdr_index,vacc_unpkd_pkt['pkt_time'],vacc_unpkd_pkt['pkt_xeng'],vacc_unpkd_pkt['pkt_offset'],vacc_unpkd_pkt['pkt_flags'],vacc_unpkd_pkt['pkt_data_len'])
