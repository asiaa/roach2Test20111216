...
function b = xps_adc_5g_dmux(blk_obj)

if ~isa(blk_obj,'xps_block')
    error('XPS_ADC class requires a xps_block class object');
end

if ~strcmp(get(blk_obj,'type'),'xps_adc_5g_dmux')
...
b = class(s,'xps_adc_5g_dmux',blk_obj);

% ip name and version
b = set(b, 'ip_name', 'adc_5g_dmux_interface');
switch s.hw_sys
    case 'ROACH'
        b = set(b, 'ip_version', '1.00.a');
end % switch s.hw_sys

supp_ip_names    = {'', 'opb_adc5g_dmux_controller'};
