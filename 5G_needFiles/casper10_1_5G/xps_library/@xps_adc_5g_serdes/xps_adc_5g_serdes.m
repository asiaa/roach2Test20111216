...
function b = xps_adc_5g_serdes(blk_obj)

if ~isa(blk_obj,'xps_block')
    error('XPS_ADC_5G_SERDES class requires a xps_block class object');
end

if ~strcmp(get(blk_obj,'type'),'xps_adc_5g_serdes')
...
b = class(s,'xps_adc_5g_serdes',blk_obj);

% ip name and version
b = set(b, 'ip_name', 'adc_5g_serdes_interface');
switch s.hw_sys
    case 'ROACH'
        b = set(b, 'ip_version', '1.00.a');
end % switch s.hw_sys

%supp_ip_names    = {'', 'opb_adc5g_serdes_controller'};
