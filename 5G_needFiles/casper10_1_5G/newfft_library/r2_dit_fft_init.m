function r2_dit_fft_init(sys,n_stages,n_bits,bin_pt,shifts,commutator_bram, ...
  twiddle_bram,n_bits_w,mult_latency,add_latency, ...
  commute_output,output_order,varargin)
% Build a Radix-2 Biplex FFT within a subsystem.
%
% r2_dit_fft_init(sys, n_stages, n_bits, bin_pt, shifts, commutator_bram, ...
%                 twiddle_bram, n_bits_w, mult_latency, ...
%                 add_latency, commute_output, output_order)
%
% sys = The subsystem to contain the FFT.  Must have 3 inports and 3 outports.
% n_stages = Number of radix-2 stages
...
%              2 = Output in normal order (uses extra resoruces).
  ...
  % Sanity check
  if length(ins) ~= 3
    msg=[sys, ' does not have 3 inports.'];
    error(msg);
  end
  if length(outs) ~= 3
    msg=[sys, ' does not have 3 outports.'];
    error(msg);
  end

  % Bail out if state matches parameters
  if ~save_state(sys,'r2_dit_fft_init',n_stages,n_bits,bin_pt,shifts,...
  ...
  annotation=sprintf('%d points\n%d_%d r/i in\n%d_%d twiddles', ...
    2^n_stages,n_bits,bin_pt,n_bits_w,n_bits_w-2);
  ...
  for k=1:3
    switch get_param(ins{k},'Port')
      ...
      case '3', set_param(ins{k},'Name','sync_in','Position',ipp+2*dipp);
    end
    switch get_param(outs{k},'Port')
      ...
      case '3', set_param(outs{k},'Name','sync_out','Position',opp+2*dopp);
    end
  end
  ...
  % Add FFT stages
  src=find_by_name('','r2_dit_stage');
  for cur_stage=1:n_stages
    blk=['r2_dit_stage_',num2str(cur_stage)];
    ...
    % Init block
    r2_dit_stage_init([sys,'/',blk],n_stages,cur_stage,n_bits,bin_pt, ...
    ...
    x=x+dx;
    n_bits=n_bits+1-shifts(cur_stage);
    bin_pt=bin_pt-shifts(cur_stage);
  end

  % Add biplex_commutator
  if commute_output > 1
    src=find_by_name('','biplex_commutator');
    blk='biplex_commutator';

    % Add block
    add_block(src,[sys,'/',blk],'Position',[x,y,x+w,y+h]);

    % Init block
    biplex_commutator_init([sys,'/',blk],n_stages,n_bits, ...
  ...
  if output_order > 1 && n_stages > 1
    src=find_by_name('','biplex_bitrev');
    blk='biplex_bitrev';
    ...
    % Init block
    biplex_bitrev_init([sys,'/',blk],n_stages-1,n_bits,varargin{:});
