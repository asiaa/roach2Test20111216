function cmpy3_init(blk,n_bits_b,bin_pt_b,n_bits_w,bin_pt_w,...
  n_bits_bw,bin_pt_bw,quantization,overflow,mult_latency,add_latency,varargin)
% Configure a cmpy3 block
%
% cmpy3_init(blk, n_bits_b, bin_pt_b, n_bits_w, bin_pt_w, ...
...
% add_latency = Latency to use for the underlying real adders.
  ...
  if ~save_state(blk,'cmpy3_init',n_bits_b,bin_pt_b,n_bits_w,bin_pt_w, ...
      n_bits_bw,bin_pt_bw,quantization,overflow, ...
  ...
  latency=mult_latency+2*add_latency+conv_latency;
  ...
  wrapables={'rere','imim','sumsum','rr_add_ii','rr_sub_ii','ss_sub_rrii'};
  if overflow == 1
  ...
  set_param(find_by_name(blk,'sumsum'),'latency',num2str(mult_latency));

  set_param(find_by_name(blk,'br_add_bi'),'latency',num2str(add_latency));
  set_param(find_by_name(blk,'wr_add_wi'),'latency',num2str(add_latency));
  set_param(find_by_name(blk,'rr_add_ii'),'latency',num2str(add_latency));

  set_param(find_by_name(blk,'rr_sub_ii'),'latency',num2str(2*add_latency));
  set_param(find_by_name(blk,'ss_sub_rrii'),'latency',num2str(add_latency));
